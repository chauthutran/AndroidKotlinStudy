...
{	
	...
	{
		// When we come back to the app for a while after the app said "User is disabled",
		// We need to check again if user is still disable or not.
		// If the user is still disable, then show the ""This user is disabled." message
		// KeycloakManager.showDialog("This user is disabled.", KeycloakManager.authenticate_WithoutToken());
	}
...
// KeycloakManager.stopOnlineService = function()
// {
// 	// Clear the checking token timeout
// 	clearTimeout(KeycloakManager.accessTokenTimeoutObj);
// 	clearTimeout(KeycloakManager.refreshTokenTimeoutObj);
...
KeycloakManager.authenticate_WithoutToken = function(successFunc, errorFunc)
{
	...
	KeycloakManager.keycloakObj.init({ 
		...
		if( !authenticated ) {
			...
			KeycloakLSManager.setKeycloakInfo_ByKeycloakObj( KeycloakManager.keycloakObj );
...
KeycloakManager.updateToken = function()
{
	...
	{
		$.ajax({
			url:  accesstokenParsed.iss + "/protocol/openid-connect/token",
			type: "POST",
			data: `grant_type=refresh_token&refresh_token=${KeycloakLSManager.getRefreshToken()}&client_id=pwaapp`,
			dataType: "json",
			success: function (response) 
			{
				if( response.error ) // Update FAILURE : { "error": "invalid_grant","error_description": "Token is not active" }
				{
					
					KeycloakManager.eventMsg( errorMsg );

					var errorMsg = response.error_description;
					if( errorMsg == "User disabled")
					{
						KeycloakManager.showDialog( errorMsg, KeycloakManager.authenticate_WithoutToken() );
					}
					else if( errorMsg == "Token is not active" )
					{
						KeycloakManager.stopWatchTokenStatusExpired_SetUpLoginForm();
					}
				}
				else // Update access token successfully
				{
					KeycloakLSManager.setProcessingAction(KeycloakLSManager.KEY_PROCESSING_ACTION_USER_DISBLED); 
					KeycloakLSManager.setKeycloakInfo_ByTokens(response.access_token, response.refresh_token, response.id_token);

					// Show "Logout button" in the bottom
					KeycloakManager.btnKeyCloakLogOutTag.prop('disabled', false);

					KeycloakManager.eventMsg('New access token is created. The refresh token will be expired in ' + response.refresh_expires_in + " second(s)" );
				}
			},
			error: function ( errMsg ) {
				console.log( 'Update token has some issue : ' + errMsg );
			}
		});	
	}
}
