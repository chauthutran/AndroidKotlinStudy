...
Validation.setUp_Events = function (formTag) // setupEventsForForm
{
	...
	formTag.find(".dataValue").each(function () 
	{
		...
		{
			// On checkbox case, on change event, always clear the validation out.
			if (inputTag.attr('type') === 'checkbox' ) Validation.setTagValidationBasic(inputTag, true );
			else Validation.checkValidations(inputTag);
		});
	});
};
...
{
	Validation.setTagValidationBasic( tag, valid );

	if (valid) tag.attr('forceValid', 'true');
	else tag.attr('forceValid', 'false');
};

Validation.setTagValidationBasic = function (tag, valid) {
	var divFieldBlockTag = tag.closest('.fieldBlock');
