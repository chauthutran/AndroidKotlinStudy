...
Action.btnTargetParentTag; // button rendering div's parent tag..  <-- if tab button, target is tab content
Action.className_btnClickInProcess = 'btnClickInProcess';

// -----------------------------
// ---- Methods ----------------

// Same level as 'render' in other type of class
Action.handleClickActions = function( btnTag, btnOnClickActions, btnTargetParentTag, blockDivTag, formDivSecTag, blockPassingData )
{		
	Action.btnTargetParentTag = btnTargetParentTag;

	// if ( formDivSecTag.attr( 'data-fields') != undefined )
	if ( Action.blockObj.blockFormObj && Action.blockObj.blockFormObj.formJsonArr )
	{
		Action.handleSequenceIncrCommits( formDivSecTag, Action.blockObj.blockFormObj.formJsonArr );
	}

	var dataPass = {};

	if ( !Action.btnClickedAlready( btnTag ) )
	{
		Action.btnClickMarked( btnTag );

		var loadingTag = FormUtil.generateLoadingTag( btnTag );

		Action.handleActionsInSync( blockDivTag, formDivSecTag, btnTag, btnOnClickActions, 0, dataPass, blockPassingData, function( finalPassData, resultStr ) 
		{
			Action.clearBtn_ClickedMark( btnTag );
...
Action.handleItemClickActions = function( btnTag, btnOnClickActions, itemIdx, blockDivTag, itemBlockTag, blockPassingData )
{		
	Action.btnTargetParentTag = Action.blockObj.parentTag; 
	...
	if ( !Action.btnClickedAlready( btnTag ) )
	{
		...
		Action.handleActionsInSync( blockDivTag, itemBlockTag, btnTag, btnOnClickActions, 0, dataPass, blockPassingData, function( finalPassData, resultStr ) {
			Action.clearBtn_ClickedMark( btnTag );					
		} );
	}
	else
...
// Action.recurrsiveActions
Action.handleActionsInSync = function( blockDivTag, formDivSecTag, btnTag, actions, actionIndex, dataPass, blockPassingData, endOfActionsFunc )
{
	if ( actionIndex >= actions.length ) endOfActionsFunc( dataPass, "Success" );
	else
	{
		// Action.clickActionPerform
		Action.actionPerform( actions[actionIndex], blockDivTag, formDivSecTag, btnTag, dataPass, blockPassingData, function( bResult, moreInfoJson )
		{			
			if ( bResult )
			{
				...
				Action.handleActionsInSync( blockDivTag, formDivSecTag, btnTag, actions, actionIndex, dataPass, blockPassingData, endOfActionsFunc );				
			}
			else
			{
...
// Action.clickActionPerform 
Action.actionPerform = function( actionDef, blockDivTag, formDivSecTag, btnTag, dataPass, blockPassingData, afterActionFunc )
{
	...
	try
	{
		...
		{				
			...
			{
				blockPassingData.displayData = Action.actionEvaluateExpression( blockPassingData.displayData, clickActionJson );
			...
			else if ( clickActionJson.actionType === "clearOtherBlocks" )
			{
				...
				Action.btnTargetParentTag.find( 'div.block' ).not( '[blockId="' + currBlockId + '"]' ).remove();
			...
			else if ( clickActionJson.actionType === "closeBlock" )
			{
				if( clickActionJson.closeLevel !== undefined )
				{
					...
					var divBlockTotal = Action.btnTargetParentTag.find( 'div.block:visible' ).length;
				...
				{
					Action.btnTargetParentTag.find("[blockid='" + clickActionJson.blockId + "']" ).remove();
				}
			...
			{		
				...
				Action.btnTargetParentTag = sheetFullL2Tag.find( '.contentBody' );
			}
			...
			else if ( clickActionJson.actionType === "openBlock" )
			{
				...
				if ( blockJson )
				{
					...
					//var targetDivTag = ( Action.blockObj.blockType === 'mainTab' ) ? '': Action.blockObj.parentTag;

					// // REMOVED: Action.blockObj.hideBlock(); { 'notClear': true }  <--- ??
					// var newBlockObj = new Block( Action.cwsRenderObj, blockJson, clickActionJson.blockId, Action.btnTargetParentTag, blockPassingData, undefined, clickActionJson );
					// newBlockObj.render();

					FormUtil.renderBlockByBlockId( clickActionJson.blockId, Action.cwsRenderObj, Action.btnTargetParentTag, blockPassingData, undefined, clickActionJson );
				}
			...
			{
				var dataFromDivTag = Action.btnTargetParentTag.find("[blockid='" + clickActionJson.fromBlockId + "']" );
				var dataToDivTag = Action.btnTargetParentTag.find("[blockid='" + clickActionJson.toBlockId + "']" );
			...
			else if ( clickActionJson.actionType === "processWSResult" ) 
			{
				...
				if ( wsReplyData && wsReplyData.resultData && clickActionJson.resultCase )
				{
					...
					if ( statusActions && statusActions.length > 0 )
					{
						...
						Action.handleActionsInSync( blockDivTag, formDivSecTag, btnTag, statusActions, 0, dataPass_Status, wsReplyData, function( finalPassData ) {
							afterActionFunc( true );
						} );
			...
			else if ( clickActionJson.actionType === "WSlocalData" )
			{
				...
				if ( clickActionJson.localResource )
				{
					...
					Action.handleActionsInSync( blockDivTag, formDivSecTag, btnTag, statusActions, 0, dataPass_Status, wsExchangeData, function( finalPassData ) {
						afterActionFunc( true );
					} );
			...
			else if ( clickActionJson.actionType === "sendToWS" && clickActionJson.redeemListInsert !== "true" )
			{
				...
				var actionUrl = Action.getActionUrl_Adjusted( clickActionJson );
				...
				Action.submitToWs( actionUrl, payloadJson, clickActionJson, btnTag, dataPass, function( bResult, optionJson ) {
			...
			else if ( clickActionJson.actionType === "queueActivity" || ( clickActionJson.actionType === "sendToWS" && clickActionJson.redeemListInsert === "true" ) )
			{
				...
				{ 
					...
					if ( passed )
					{
						...
						var dupVoucherActivityPass = ( ConfigManager.getVoucherCodeReuse() || Action.checkDuplicate_VoucherTransActivity( formsJsonActivityPayload, editModeActivityId ) );
...
Action.checkDuplicate_VoucherTransActivity = function( formsJsonActivityPayload, editModeActivityId )
{
...
Action.getActionUrl_Adjusted = function( actionDefJson )
{
	return ( actionDefJson.dws && actionDefJson.dws.url ) ? actionDefJson.dws.url : actionDefJson.url;
};


Action.submitToWs = function( actionUrl, payloadJson, actionDefJson, btnTag, dataPass, returnFunc )
{
	...
	{					
		...
		WsCallManager.wsActionCall( actionUrl, payloadJson, undefined, function( success, redeemReturnJson ) {
			...
			if (success && ( redeemReturnJson.status === "success" || redeemReturnJson.status === "pass" ) )
			{
				...
				if ( actionDefJson.searchType === 'voucherSearch' ) Action.handleMultipleVouchersSplit(redeemReturnJson, payloadJson );
...
Action.actionEvaluateExpression = function( jsonList, actionExpObj )
{
	...
	if ( actionExpObj.expressionNew ) Action.runNewEvaluateExpression( actionExpObj, jsonList );
	else
	{
		...
		var expectedFieldList = Action.getUniqueFieldListFromEvaluateExpression( myCondTest );

		for( var i = 0; i < jsonList.length; i++ )
		{
			// temp disabled compareExpressionFieldList_withDataFieldList() { 'containsExpectedFields': true }; //
			var fieldComparison = { 'containsExpectedFields': true }; //Action.compareExpressionFieldList_withDataFieldList( expectedFieldList, jsonList[ i ] );
...
Action.runNewEvaluateExpression = function( actionExpObj, jsonList )
{
...
Action.getUniqueFieldListFromEvaluateExpression = function( actionExpObj )
{
...
Action.compareExpressionFieldList_withDataFieldList = function( exprFieldsArray, recordArray )
{
...
Action.btnClickedAlready = function( btnTag )
{
	return ( btnTag && btnTag.hasClass( Action.className_btnClickInProcess ) );
};

Action.btnClickMarked = function( btnTag )
{
	btnTag.addClass( Action.className_btnClickInProcess );
};

Action.clearBtn_ClickedMark = function( btnTag )
{
	btnTag.removeClass( Action.className_btnClickInProcess );
};

// ========================================================

Action.handleSequenceIncrCommits = function( formDivSecTag, jData )
{
...
Action.handleMultipleVouchersSplit = function( responseJson, payloadJson )
{
	try
	{
